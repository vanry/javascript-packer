...
class JavascriptPacker
{
    ...
    public function __construct($script = null, $encoding = 62, $fastDecode = true, $specialChars = false)
    {
        $this->script = $script;
    ...
    public function pack($script = null)
    {
        if (! is_null($script)) {
            $this->script = $script;
        }
    ...
    private function _encodeKeywords($script)
    {
        ...
        // create the parser
        $parser = new ParseMaster;
        $encode = $this->_getEncoder($this->encoding);
    ...
    private function _bootStrap($packed, $keywords)
    {
        ...
        $encode = ($this->encoding > 62) ? '_encode95' : $this->_getEncoder($ascii);
        $encode = $this->_getJSFunction($encode);
        $encode = preg_replace('/encoding/', '$ascii', $encode);
        ...
        if ($this->encoding > 62) { // high-ascii
    ...
    private function _encode62($charCode)
    {
        ...
        if ($charCode >= $this->encoding) {
            $res = $this->_encode62((int)($charCode / $this->encoding));
        }
    ...
    private function _encode95($charCode)
    {
        ...
        if ($charCode >= $this->encoding) {
            $res = $this->_encode95($charCode / $this->encoding);
        }
...
'function($charCode) {
    return ($charCode < encoding ? \'\' : arguments.callee(parseInt($charCode / encoding))) +
    (($charCode = $charCode % encoding) > 35 ? String.fromCharCode($charCode + 29) : $charCode.toString(36));
}';
...
'function($charCode) {
    return ($charCode < encoding ? \'\' : arguments.callee($charCode / encoding)) +
        String.fromCharCode($charCode % encoding + 161);
}';
}
